...
RSpec.describe Item, type: :model do
  ...
  describe '出品する'  do
    context 'ユーザーが出品できる時' do
      it 'ユーザーがログインしており、商品画像、商品名、商品の説明、カテゴリーと商品の状態、配送料の負担と発送元の地域発送までの日数、販売価格を入力すれば出品できること' do
    ...
    context 'ユーザーが出品できない時' do
      ...
      it '商品の状態は、「新品・未使用、未使用に近い、目立った傷や汚れなし、やや傷や汚れあり、傷や汚れあり、全体的に状態が悪い」の6項目から選択肢しないと出品できない'do
       @item.condition_id = 1
       @item.valid?
       expect(@item.errors.full_messages).to include "Condition can't be blank"
      end
      ...
      it '配送料の負担の情報が空だと出品できない' do
       ...
       it '配送料の負担は、「着払い(購入者負担)、送料込み(出品者負担)」の2項目から選択しないと出品できない' do
      ...
      it '発送元の地域は、47都道府県から選択しないと出品できない' do
      ...
      it '発送までの日数の情報が空だと出品できない' do
       ...
       it '発送までの日数は、「1~2日で発送、2~3日で発送、4~7日で発送」の3項目から選択しないと出品できない' do
        @item.duration_id = 1
        @item.valid?
        expect(@item.errors.full_messages).to include "Duration can't be blank"
       end
